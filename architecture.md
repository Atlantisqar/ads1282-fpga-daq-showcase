...
> 我把项目拆成采集、缓存、传输、记录、显示和诊断几个层次。FPGA 侧负责实时性和确定性，PC 侧负责交互、可视化和数据分析。调试时我尽量让每个异常都能在状态、LED、示波器或抓包中找到对应证据，而不是靠猜。

不建议公开：

> 完整寄存器写入序列、完整协议字段、完整状态机跳转条件、完整引脚表、私有网络参数和可直接复刻的底层时序。
